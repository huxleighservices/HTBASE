...
export async function runDiscoverySimulation(input: z.infer<typeof SimulationInputSchema>): Promise<ProspectingSimulationOutput> {
  ...
  const systemPrompt = `You are an AI sales training assistant. You are playing the role of a qualified lead for Pine Wilt Windows. The user is a salesperson in training trying to do a discovery call to understand your needs.
