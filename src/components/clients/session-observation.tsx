...
import {
  ...
  // This component is nearly identical to SessionManager, but it queries all sessions
  // for a client instead of just one.
...
export function SessionObservation({ clientPath }: { clientPath: string }) {
    const firestore = useFirestore();
    const { toast } = useToast();
    
    const sessionsCollectionRef = useMemoFirebase(() => {
      if (!firestore || !clientPath) return null;
      return collection(firestore, clientPath, 'sessions');
    }, [firestore, clientPath]);
  
    const { data: allSessions, isLoading } = useCollection<{id: string, results?: TrainingResult[]}>(sessionsCollectionRef);
  
    const handleDeleteResult = async (sessionId: string, resultToRemove: TrainingResult) => {
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center gap-3">
            ...
            <CardTitle className="font-headline text-lg">Session Observation</CardTitle>
          </div>
          <CardDescription className="pt-2">
            Review past training simulations and performance feedback for all users of this client.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading && (
          ...
          {!isLoading && (!allSessions || allSessions.length === 0) && (
            <div className="flex h-24 items-center justify-center rounded-lg border-2 border-dashed bg-muted/50 text-muted-foreground">
              <p>No training sessions found for this client.</p>
            </div>
          )}
          {!isLoading && allSessions && allSessions.length > 0 && (
            <Accordion type="single" collapsible className="w-full">
              {allSessions.map((session) => {
                const sortedResults = session.results?.sort((a, b) => (b.createdAt as any) - (a.createdAt as any)) || [];
                if (sortedResults.length === 0) return null;
                
                return (
                <AccordionItem key={session.id} value={`session-${session.id}`}>
                    <AccordionTrigger>
                        <p className="font-semibold text-left">Trainee: {session.id}</p>
                    </AccordionTrigger>
                    <AccordionContent className="space-y-4">
                        {sortedResults.map((result, index) => (
                             <Accordion key={index} type="single" collapsible className="w-full bg-background/50 rounded-lg px-4">
                                <AccordionItem value={`result-${index}`} className="border-b-0">
                                         ...
                                         <AlertDialog>
                                             ...
                                             <AlertDialogContent>
                                                 ...
                                                 <AlertDialogDescription>
                                                     This will permanently delete this training result for trainee {session.id}. This action cannot be undone.
                                                 </AlertDialogDescription>
                                                 </AlertDialogHeader>
                                                 <AlertDialogFooter>
                                                 <AlertDialogCancel>Cancel</AlertDialogCancel>
                                                 <AlertDialogAction onClick={() => handleDeleteResult(session.id, result)} className={cn(
