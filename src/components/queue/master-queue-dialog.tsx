...
import type { Client, QueueTask } from '@/types/client';
import { GanttChartSquare, CheckCircle, XCircle, Info } from 'lucide-react';
...
import { collection, query, doc, Timestamp, arrayUnion, serverTimestamp, orderBy } from 'firebase/firestore';
...
const IncompleteNoteDialog = ({ open, onOpenChange, onSubmit }: { open: boolean, onOpenChange: (open: boolean) => void, onSubmit: (data: IncompleteFormValues) => void }) => {
    ...
    return (
        <AlertDialog open={open} onOpenChange={onOpenChange}>
            <AlertDialogContent>
                <Form {...form}>
                    <form id="incomplete-note-form" onSubmit={form.handleSubmit(onSubmit)}>
                        <AlertDialogHeader>
                 ...
                 <AlertDialogFooter>
                    ...
                    <Button type="submit" form="incomplete-note-form">Save & Set Reminder</Button>
                </AlertDialogFooter>
            </AlertDialogContent>
        </AlertDialog>
    );
...
export function MasterQueueDialog({ open, onOpenChange, client }: MasterQueueDialogProps) {
            ...
            <Card key={task.id}>
                <CardContent className="p-4 flex items-center justify-between">
                     <div className="flex items-center gap-4">
                        ...
                        <div>
                            <p className="font-semibold">{task.leadName} <span className="text-xs font-normal text-muted-foreground">({task.agent})</span></p>
      ...
      <DialogContent className="max-w-4xl h-[80vh] flex flex-col">
        <DialogHeader>
          <DialogTitle className="text-3xl font-bold font-headline tracking-tight flex items-center gap-3">
            <GanttChartSquare /> Master Queue
          </DialogTitle>
          <DialogDescription>
            A combined view of all automated lead follow-up reminders.
          </DialogDescription>
        </DialogHeader>
