...
import { FileText, Send, Loader2, Sparkles, ThumbsUp, Lightbulb, Trash2, Bot, User, Save } from 'lucide-react';
...
import { runProposalSimulation } from '@/ai/flows/proposal-simulation-flow';
...
export function ProposalSimulatorDialog({ open, onOpenChange, activeSessionId, clientPath, trainingData }: { open: boolean, onOpenChange: (open: boolean) => void, activeSessionId: string | null, clientPath: string | null, trainingData?: string }) {
    ...
    const handleSendMessage = async (e: FormEvent) => {
        ...
        try {
            const result = await runProposalSimulation({
                difficulty,
            ...
            >
                <DialogHeader>
                    <DialogTitle className="flex items-center gap-3 text-2xl">
                        ...
                        Tr/AI/ner: Proposal Simulation
                    </DialogTitle>
                    <DialogDescription>
                        Practice presenting a plan and its value. Your goal is to get the prospect to agree to the proposal and move to the next step. You have {USER_MESSAGE_LIMIT} messages.
                    </DialogDescription>
                </DialogHeader>
                ...
                <div className="flex-grow grid grid-cols-1 md:grid-cols-3 gap-6 overflow-hidden">
                    ...
                    <div className="md:col-span-1 flex flex-col gap-4 p-1 min-h-0">
                        <div className="space-y-2">
                            <Label htmlFor="difficulty-select">Prospect's Attitude</Label>
                            <Select 
                                value={difficulty} 
                            ...
                            >
                                ...
                                <SelectContent>
                                    <SelectItem value="Easy">Ready to Buy 😊</SelectItem>
                                    <SelectItem value="Medium">Value-Conscious 🤔</SelectItem>
                                    <SelectItem value="Hard">Sticker Shock 😠</SelectItem>
                                </SelectContent>
                            </Select>
                        </div>
                                ...
                                <ScrollArea className="h-full pr-3 -mr-3">
                                    {!feedback ? (
                                        <div className="flex items-center justify-center h-full text-center text-muted-foreground">
                                            <p>Complete the simulation to receive feedback on your proposal skills.</p>
                                        </div>
                                    ) : (
