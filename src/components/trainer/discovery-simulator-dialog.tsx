...
import { Microscope, Send, Loader2, Sparkles, ThumbsUp, Lightbulb, Trash2, Bot, User, Save } from 'lucide-react';
...
import { runDiscoverySimulation } from '@/ai/flows/discovery-simulation-flow';
...
export function DiscoverySimulatorDialog({ open, onOpenChange, activeSessionId }: { open: boolean, onOpenChange: (open: boolean) => void, activeSessionId: string | null }) {
    ...
    const handleSendMessage = async (e: FormEvent) => {
        ...
        try {
            const result = await runDiscoverySimulation({
                difficulty,
            ...
            >
                <DialogHeader>
                    <DialogTitle className="flex items-center gap-3 text-2xl">
                        ...
                        Tr/AI/ner: Discovery Simulation
                    </DialogTitle>
                    <DialogDescription>
                        Practice uncovering a client's needs. Your goal is to ask effective questions to understand their pain points and match them to a solution. You have {USER_MESSAGE_LIMIT} messages.
                    </DialogDescription>
                </DialogHeader>
                    ...
                    <div className="md:col-span-1 flex flex-col gap-4 p-1 min-h-0">
                        <div className="space-y-2">
                            ...
                            >
                                ...
                                <SelectContent>
                                    <SelectItem value="Easy">Cooperative & Open 😊</SelectItem>
                                    <SelectItem value="Medium">Vague & Unsure 🤔</SelectItem>
                                    <SelectItem value="Hard">Guarded & Resistant 😠</SelectItem>
                                </SelectContent>
                            </Select>
                        </div>
                                ...
                                <ScrollArea className="h-full pr-3 -mr-3">
                                    {!feedback ? (
                                        <div className="flex items-center justify-center h-full text-center text-muted-foreground">
                                            <p>Complete the simulation to receive feedback on your discovery skills.</p>
                                        </div>
                                    ) : (
