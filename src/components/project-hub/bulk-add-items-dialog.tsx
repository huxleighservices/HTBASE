...
export function BulkAddItemsDialog({
  ...
  const [editedItems, setEditedItems] = useState<Omit<ProjectItem, 'id'>[]>([]);
  ...
  const handleNextToMap = () => {
    ...
    initialHeader.forEach((cell, index) => {
        ...
        const matchingProjectColumn = project.columns.find(projCol => projCol.toLowerCase().replace(/\s/g, '') === lowerCell);
        if (matchingProjectColumn) {
          newColumnMap[index] = matchingProjectColumn;
        } else {
          newColumnMap[index] = 'ignore';
        }
    });
  ...
  const handleNextToConfirm = () => {
    setEditedItems(getMappedItems());
    setStage('confirm');
  }

  const handleItemChange = (itemIndex: number, column: string, value: string) => {
  ...
  const handleBulkAdd = async () => {
    ...
    try {
        await batch.commit();
        toast({ title: "Success!", description: `${itemsToAdd.length} items added to project '${project.name}'.` });
        onOpenChange(false);
    } catch (error: any) {
      ...
      <DialogContent className="sm:max-w-4xl max-h-[90vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            {stage === 'map' && 'Map your columns to the project fields.'}
        ...
        <div className="flex-grow overflow-y-auto pr-2">
            ...
            {stage === 'map' && (
                <div className="space-y-4">
                    <p className="text-sm text-muted-foreground">The first row of your data is used as headers. Map them to the corresponding fields for this project. Select 'Ignore' for columns you don't want to import.</p>
            ...
            {stage === 'confirm' && editedItems.length > 0 && (
                 <div className="border rounded-md">
                     <Table>
                        <TableHeader>
                            <TableRow>
                                {project.columns.map(field => <TableHead key={field}>{field}</TableHead>)}
                            </TableRow>
                        </TableHeader>
                        <TableBody>
                                ...
                                <TableRow key={itemIndex}>
                                    {project.columns.map(col => (
                                        <TableCell key={col}>
                                            <Input 
                                                value={item.data[col] || ''}
                                                onChange={(e) => handleItemChange(itemIndex, col, e.target.value)}
                                                className="h-8"
                                            />
                                        </TableCell>
                                    ))}
        ...
        <DialogFooter className="pt-4">
          ...
          {stage === 'map' && (
            <div className="flex gap-2">
                ...
                <Button onClick={handleNextToConfirm}>Review <ArrowRight className='ml-2'/></Button>
            </div>
          )}
          {stage === 'confirm' && (
            <div className="flex gap-2">
                ...
                <Button onClick={handleBulkAdd} disabled={isProcessing}>
                    ...
                    {isProcessing ? 'Importing...' : `Import ${editedItems.length} Items`}
                </Button>
            </div>
          )}
        </DialogFooter>
